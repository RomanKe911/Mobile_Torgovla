 ...
 */
public class PrefActivity_Splash_New extends PreferenceActivity {
    ...
    public Preference button_load_start, button_users;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        addPreferencesFromResource(R.xml.pref_splash_new);
        context_Activity = PrefActivity_Splash_New.this;
      ...
      /*  Permission_Loading_Start();
        ...
        button_id_catigory.setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            ...
            public boolean onPreferenceClick(Preference preference) {
                ...
                PrefActivity_Splash_New.MyAsyncTask_Sync_FTP_CONST asyncTask = new PrefActivity_Splash_New.MyAsyncTask_Sync_FTP_CONST();
                asyncTask.execute();
            ...
            public boolean onPreferenceClick(Preference preference) {
                ...
                try {
                    ...
                    if (pInfo.versionCode != number_update) {
                        ...
                        PrefActivity_Splash_New.MyAsyncTask_Sync_Update_Create_Table asyncTask = new PrefActivity_Splash_New.MyAsyncTask_Sync_Update_Create_Table();
                        asyncTask.execute();
    ...
    private class MyAsyncTask_Sync_FTP extends AsyncTask<Void, Integer, Void> {
        ...
        protected void onPostExecute(Void aVoid) { // Вызывается в конце потока
           ...
           /* Toast.makeText(context_Activity, "Загруженно на " + this_data_now + ": \nXML:" + files_kol + " " + files_size +
            ...
            web.ftp_server = PEREM_FTP_SERV;
            web.ftp_port = 21;
            web.ftp_user_name = PEREM_FTP_LOGIN;
            web.ftp_password = PEREM_FTP_PASS;
               /* web.put_toFiles = "/MT_Sunbell_Bishkek/MTW_Data/";
                web.name_File = "MTW_In_Brends.xml";*/



            String stroka1, stroka2;
            web.put_toFiles = "/MT_Sunbell_Karakol/MTW_Data/";
            stroka1 = web.getFileExtension(true);
            web.put_toFiles = "/MT_Sunbell_Karakol/SqliteDB/";
            stroka2 = stroka1+ web.getFileExtension(true);
            Log.e("HASH...", "Список файлов?: " + stroka2); // Запрос даты файла

            button_load_start.setSummary(stroka2);


           /* button_load_start.setSummary("Данные на: " + this_data_now + ": \nXML: " + files_kol + " " + files_size +
                    "\nБД: " + files_kol_sql + " " + files_size_sql);*/
            ed = sp.edit();
            ed.putString("Data_dialog_sp_1", button_load_start.getSummary().toString()); // обновление агентов
            ed.commit();
            pDialog.setProgress(0);
            pDialog.cancel();
            pDialog.dismiss();
        ...
        private void getFloor() throws InterruptedException {
            ...
            try {
               ...
               /* Log.e("CONN...", "FTP1: " + web.getDataFile()); // Запрос даты файла


                //  web.put_toCatalogFiles = "/MT_Sunbell_Bishkek/MTW_Data";
                web.put_toCatalogFiles = "/MT_Sunbell_Bishkek";
                // MT_Sunbell_Karakol/MTW_Data
                Log.e("CONN...", "FTP2: " + web.getFilesSize()); // Запрос даты файла*/



                //  web.getFileToPhone("/MT_Sunbell_Karakol/02_WDay/", getApplicationContext()); // Скачивание, файлов

               /* String file_Phone = Environment.getExternalStorageDirectory().toString() + "/Price/XML/";
                String put_start = file_Phone;
                String put_end = "/MT_Sunbell_Karakol/Image/";
                web.getFileToFTP(put_start, put_end, getApplicationContext());*/
               // web.getFileExtension("/MT_Sunbell_Karakol/Image/", true, true);


                //Log.e("HASH...", "Список файлов?: " + ); // Запрос даты файла

               /* String wer = web.getFileExtension( true);
                Log.e("HASH...", "Список файлов?: " + wer); // Запрос даты файла
                Toast.makeText(getApplicationContext(), wer, Toast.LENGTH_SHORT).show();*/
                // /MT_Sunbell_Karakol/MTW_Data


               /* HashSet<String> countryHashSet = new HashSet<>();
                countryHashSet = (web.getListFile_SOSDB("toktobaeva_elena" + ".db3", "/MT_Sunbell_Bishkek/MTW_SOS/"));
                String[] myArray = {};
                myArray = countryHashSet.toArray(new String[countryHashSet.size()]);
                for (int i = 0; i < myArray.length; i++) {
                    Log.e("HASH...", "F: " + myArray[i]); // Запрос даты файла
                }*/

               /* TreeMap treeMap = web.getListFile_SOSDB("toktobaeva_elena" + ".db3", "/MT_Sunbell_Bishkek/MTW_SOS/");
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                    treeMap.entrySet().forEach(System.out::println);
                    System.out.println("Нераспаршена с помощью2 ");
                    Log.e("treeMap...", "Размер: " + treeMap.size()); // Запрос даты файла
                    for (int i=0; i<=treeMap.size(); i++)
                    {
                        Log.e("treeMap...", "F: " + treeMap.values()); // Запрос даты файла
                    }
                }*/
                ...
                try {
                    ...
                    FTPFile dataFile = ftpClient.mdtmFile("/MT_Sunbell_Bishkek/MTW_Data/MTW_In_Brends.xml");
                    FTPFile dataFile2 = ftpClient.mdtmFile("/MT_Sunbell_Karakol/MTW_Data/MTW_In_Brends.xml");
                    int f = ftpClient.mdtm("/MT_Sunbell_Karakol/MTW_Data/MTW_In_Brends.xml");

                    Log.e("ДАТА файла...", "ДАТА1: " + dataFile);
                    Log.e("ДАТА файла...", "ДАТА2: " + dataFile2);
                    Log.e("ДАТА файла...", "ДАТА3: " + f);

                   /* Log.e("ДАТА файла...", "Д1: " + dataFile2.isDirectory());
                    Log.e("ДАТА файла...", "Д1: " + dataFile2.getSize());
                    Log.e("ДАТА файла...", "Д1: " + dataFile2.getName()); // Имя файла
                    Log.e("ДАТА файла...", "Д1: " + dataFile2.getLink());
                    Log.e("ДАТА файла...", "Д1: " + dataFile2.getTimestamp().getTime()); // Wed Jan 11 05:22:23 GMT+06:00 2023
                    */

                    SimpleDateFormat sdf1 = new SimpleDateFormat("yyyy.MM.dd HH.mm.ss");
                    Log.e("ДАТА файла...", "Бишкек: " + sdf1.format(dataFile.getTimestamp().getTime()));
                    Log.e("ДАТА файла...", "Каракол: " + sdf1.format(dataFile2.getTimestamp().getTime()));
                    Log.e("ДАТА файла...", "NEW1: " + dataFile2.isValid());
                    Log.e("ДАТА файла...", "NEW1: " + dataFile2.isFile());
                    Log.e("ДАТА файла...", "NEW1: " + dataFile2.toFormattedString());


                    //FTP-Bishkek: 2023-01-11 11:04:53
                    //FTP-Karakol: 2023-01-11 05:22:23

                    // LOGE-Bishkek 2023-01-11 05:04:53
                    // LOGE-Karakol 2023-01-10 23:22:23

                    System.out.println("Total dirs: " + dirInfo[0]);
                    System.out.println("Total files: " + dirInfo[1]);
                    System.out.println("Total size: " + dirInfo[2]);
                    System.out.println("Total ЕЕЕ: " + dirInfo[3]);
                    System.out.println("Total WWW: " + dirInfo[4]);
                  ...
                  /*  ftpClient.enterLocalPassiveMode();
                    ...
                    for (FTPFile ftpFile_XML : ftpFiles_list_XML) {
                        ...
                        String file_db = PrefActivity_Splash_New.this.getDatabasePath(ftpFile_XML.getName()).getAbsolutePath(); // путь к databases
                    ...
                    for (FTPFile ftpFile_db3 : ftpFiles_list_db3) {
                        ...
                        String file_sd = Environment.getExternalStorageDirectory().toString() + "/Price/XLS_KerkinR/";  // путь к базам данных на телефоне
                        String file_db = PrefActivity_Splash_New.this.getDatabasePath(ftpFile_db3.getName()).getAbsolutePath(); // путь к databases
        ...
        private void getFloor() throws InterruptedException {
            ...
            try {
                ...
                try {
                    ...
                    Log.e("Дирикторий=", " = " + dirInfo[0]);
                    Log.e("Файлов=", " = " + dirInfo[1]);
                    Log.e("Размер!2=", " = " + dirInfo[2] + " байт");
                    Log.e("Размер!2=", " = " + dirInfo[2] / 1024 + " кбайт");
                    Log.e("Размер!2=", " = " + dirInfo[2] / 1048576 + " мбайт");
                    ...
                    String file_db = PrefActivity_Splash_New.this.getDatabasePath("sunbell_const_db.db3").getAbsolutePath(); // путь к databases
        ...
        private void getFloor() throws InterruptedException {
            ...
            try {
                ...
                try {
                    ...
                    Log.e("Дирикторий=", " = " + dirInfo[0]);
                    Log.e("Файлов=", " = " + dirInfo[1]);
                    Log.e("Размер!3=", " = " + dirInfo[2] + " байт");
                    Log.e("Размер!3=", " = " + dirInfo[2] / 1024 + " кбайт");
                    Log.e("Размер!3=", " = " + dirInfo[2] / 1048576 + " мбайт");
                  ...
                  /*  ftpClient.enterLocalPassiveMode();
                    ...
                    Log.e("Дирикторий=", " = " + dirInfo_db3[0]);
                    Log.e("Файлов=", " = " + dirInfo_db3[1]);
                    Log.e("Размер!4=", " = " + dirInfo_db3[2] + " байт");
                    Log.e("Размер!4=", " = " + dirInfo_db3[2] / 1024 + " кбайт");
                    Log.e("Размер!4=", " = " + dirInfo_db3[2] / 1048576 + " мбайт");
...
ftpClient.enterLocalPassiveMode();
    ...
    private class MyAsyncTask_Sync_Update_Create_Table extends AsyncTask<Void, Integer, Void> {
        ...
        protected void onPostExecute(Void aVoid) { // Вызывается в конце потока
            ...
            PrefActivity_Splash_New.MyAsyncTask_Sync_Update_Create_Rows asyncTask = new PrefActivity_Splash_New.MyAsyncTask_Sync_Update_Create_Rows();
            asyncTask.execute();
        }
        ...
        switchPreference_permission.setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            ...
            public boolean onPreferenceClick(Preference preference) {
             /*   if (switchPreference_permission.isChecked()) {
                ...
                if (PermissionUtils.hasPermissions(PrefActivity_Splash_New.this)) return true;
                PermissionUtils.requestPermissions(PrefActivity_Splash_New.this, PERMISSION_STORAGE);
                return false;
            }
        });
    }
    ...
    protected void Permission_Server() {
       /*
        ...
        // Запись данных в настройки подключения к FTP-серверу
        // определение ключей
        ...
        button_load_start = (Preference) findPreference("button_load_start");

        // настройка ip ftp-сервера
        ...
        // настройка путей к данным ftp-сервера
        ...
        listPreference_ftp_con.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
            ...
            public boolean onPreferenceChange(Preference preference, Object newValue) {
                ...
                if (preference instanceof ListPreference) {
                    ...
                    preference.setSummary(
                            index >= 0
                                    ? "ip-адрес-> " + listPreference.getEntries()[index]
                                    : "По умолчанию");
                }
                return true;
            }
        ...
        // проверка доступа к ftp-серверу и загрузка стартовых данных

        button_load_start.setSummary(sp.getString("Data_dialog_sp_1", "0"));
        button_load_start.setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            @Override
            public boolean onPreferenceClick(Preference preference) {
                Constanta_Read();
                if (!sp.getString("list_preference_ftp", "0").equals("1")) {
                    ...
                    PrefActivity_Splash_New.MyAsyncTask_Sync_FTP asyncTask = new PrefActivity_Splash_New.MyAsyncTask_Sync_FTP();
                    asyncTask.execute();
                } else {
        ...
        sp = PreferenceManager.getDefaultSharedPreferences(context_Activity);
        Log.e(TAG, "PERF_IP:" + sp.getString("ftp_ipConn_list", "0"));
        Log.e(TAG, "PERF_PUT:" + sp.getString("ftp_put_list", "0"));
    }
