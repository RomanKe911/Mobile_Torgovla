    ...
    public void execute() {
        ...
        Runnable runnable = () -> {
            try {
                ...
                Log.e("SkladName", preferencesMtSetting.readSettingString(getApplication(), preferencesMtSetting.getSkladName()));
                Log.e("SkladUID", preferencesMtSetting.readSettingString(getApplication(), preferencesMtSetting.getSkladUID()));
                //  pDialog.setMessage("Загрузка продуктов. Подождите...");
                SQLiteDatabase db = getApplication().getBaseContext().openOrCreateDatabase(mSettings_base_Base, MODE_PRIVATE, null);
                String query;
                if (!preferencesMtSetting.readSettingString(getApplication(), preferencesMtSetting.getStatusOrder()).equals("Edit")) {
                    ...
                    query = "SELECT base_in_nomeclature.name, base_in_nomeclature.brends, " +
                            ...
                            "LEFT JOIN const_sklad ON base_in_ostatok.sklad_uid = const_sklad.sklad_uid\n" +
                            "WHERE count > 0 AND lower(base_in_nomeclature.brends) == '" + sp_BREND.toLowerCase()+ "' " +
                            "AND base_in_ostatok.sklad_uid = '"+preferencesMtSetting.readSettingString(getApplication(), preferencesMtSetting.getSkladUID())+"'\n" +
                            "ORDER BY base_in_nomeclature.brends, base_in_nomeclature.p_group, base_group_sql.type_group ASC;";
                }
                ...
                while (cursor.isAfterLast() == false) {
                    ...
                    if (brends.equals(sp_BREND)) {
                        if (select_menu.equals(sub_brends)) {
                            arrayList_tovar.add(new ListAdapterSimple_Suncape_Forma(koduid, kod_univ, name, kolbox, cena, Cena_for_DB(cena).first, strih, ostatok, SelectImage(image), name_sklad, uid_sklad));
                            cursor.moveToNext();
...
/*                backup_list.clear();
    ...
    //// проверка на наличие картинки в базе
    protected String SelectImage(String s_image) {
        if (s_image != null) return s_image;
