...
public class WJ_Dialog_RN_Edit extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // setContentView(R.layout.mt_wj_dialog_rn_edit);
        context_Activity = WJ_Dialog_RN_Edit.this;
        Constanta_Read();
        ...
        localBuilder.setCancelable(false).setIcon(R.drawable.icon_korz).setPositiveButton(" ", new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface paramDialogInterface, int paramInt) {
                ...
                if (kol_text > 0) {
                    ...
                    ReWrite_Table_RN(lst_tw_kod, dg_tw_kol.getText().toString(), dg_tw_cena.getText().toString(), postclass_Skidka, postclass_OLD_SUMMA);
    ...
    protected void ReWrite_Table_RN(String rw_kodUID, String rw_newKOL, String
            rw_CENA, String rw_SKIDKA, String rw_old_summa) {
        ...
        ContentValues localContentValuesUP_Debet = new ContentValues();
         // Изменение количества товара и изменнеия суммы и итого
        try {
        ...
        {
            Log.e("Error:/", "Ошибка изменения кол-ва товара!");
            Toast.makeText(this, "Ошибка, изменения кол-ва товара!", Toast.LENGTH_SHORT).show();
        }

        // Изменение в базе общей суммы!
        try {
            ...
            String all_summa = cursor_rn_All.getString(cursor_rn_All.getColumnIndexOrThrow("newsumma"));
            all_itogo = cursor_rn_All.getString(cursor_rn_All.getColumnIndexOrThrow("newitogo"));
        ...
        {
            Log.e("Error:/", "Ошибка изменения общей суммы!");
            Toast.makeText(this, "Ошибка, изменения общей суммы!", Toast.LENGTH_SHORT).show();
        }

        // Изменение задолжности после изменнеия кол-ва товара
        try {
            ...
            if (cursor_debet_count.getCount()>0 & cursor_debet_count != null)
            {
                Log.e("Cursor", "YES, cursor_debet_count");

                String debet = cursor_debet_count.getString(cursor_debet_count.getColumnIndexOrThrow("d_summa"));
            ...
            } else Log.e("Cursor", "NO, cursor_debet_count");
            cursor_debet_count.close();
        }
        catch (Exception e)
        {
            Log.e("Error:/", "Ошибка изменения общей задолжности!");
            Toast.makeText(this, "Ошибка, изменения общей задолжности!", Toast.LENGTH_SHORT).show();
        }
    ...
    protected void ListAdapter() {
        ...
        while (cursor.isAfterLast() == false) {
            db_kod_rn = cursor.getString(cursor.getColumnIndexOrThrow("Kod_RN"));
            db_summa = cursor.getString(cursor.getColumnIndexOrThrow("Summa"));
            db_itogo = cursor.getString(cursor.getColumnIndexOrThrow("Itogo"));
            db_itogo = cursor.getString(cursor.getColumnIndexOrThrow("Skidka"));

            String db_ls_kod_rn = cursor.getString(cursor.getColumnIndexOrThrow("Kod_RN"));
            String db_ls_name = cursor.getString(cursor.getColumnIndexOrThrow("Name")); //
            String db_ls_kol = cursor.getString(cursor.getColumnIndexOrThrow("Kol")); //
            String db_ls_cena = cursor.getString(cursor.getColumnIndexOrThrow("Cena")); //
            String db_ls_cena_SK = cursor.getString(cursor.getColumnIndexOrThrow("Cena_SK")); //
            String db_ls_summa = cursor.getString(cursor.getColumnIndexOrThrow("Summa"));
            String db_ls_skidka = cursor.getString(cursor.getColumnIndexOrThrow("Skidka"));
            String db_ls_itogo = cursor.getString(cursor.getColumnIndexOrThrow("Itogo"));
            String db_ls_image = cursor.getString(cursor.getColumnIndexOrThrow("Image")); //
            String db_ls_koduid = cursor.getString(cursor.getColumnIndexOrThrow("koduid")); //
    ...
    protected void SQL_ReWrite_Ostatok(String sql_koduid, Integer rw_old_kol, Integer rw_new_kol, String rw_ostatok) {
        ...
        {
            Log.e("Error:/", "Ошибка не коректные остатки!");
            Toast.makeText(this, "Ошибка, не коректные остатки!", Toast.LENGTH_SHORT).show();
        }
