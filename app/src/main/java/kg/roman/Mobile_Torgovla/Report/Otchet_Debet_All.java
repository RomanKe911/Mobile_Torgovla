...
public class Otchet_Debet_All extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        context_Activity = Otchet_Debet_All.this;
        ...
        try {
            Constanta_Read();
            Otchet_Debet_All.MyAsyncTask_Sync_Debet asyncTask = new  Otchet_Debet_All.MyAsyncTask_Sync_Debet();
            asyncTask.execute();
    ...
    protected void Loading_Debet() {
        ...
        String query = "SELECT * FROM otchet_debet\n" +
                "WHERE d_agent_uid = '" + PEREM_AG_UID + "' AND d_summa > 0";

        final Cursor cursor = db.rawQuery(query, null);
        cursor.moveToFirst();
        Double aut_summa = 0.0;
        while (cursor.isAfterLast() == false) {
            String client = cursor.getString(cursor.getColumnIndex("d_kontr_name"));
            String client_uid = cursor.getString(cursor.getColumnIndex("d_kontr_uid"));
            String summa = cursor.getString(cursor.getColumnIndex("d_summa")).replaceAll(",", ".");

            Log.e("Return1", client);
            Log.e("Return2", summa);
            Log.e("Return2", aut_summa.toString());
            aut_summa = aut_summa + Double.parseDouble(summa);

            debet.add(new ListAdapterSimple_Debet(client, client_uid, summa, ""));
            cursor.moveToNext();
        }
        cursor.close();
        db.close();
